...
function selectPiece(event) {
  ...
  let optionalmovements = checkOptionalmovements(type, {
  ...
  piecesArr.forEach((piece) => {
    if (
      piece.position.i == objectifier.i &&
      piece.position.j == objectifier.j
    ) {
      clickedpiece = piece;
    }
  });
  ...
  authenticatedMovements = movementAuthentication(
    optionalmovements,
    clickedpiece
  );
...
function movePiece(event) {
  ...
  piecesArr.map((piece, index) => {
    if (
      piecesArr[index].position.i == parseInt(newSL[0], 10) &&
      piecesArr[index].position.j == parseInt(newSL[1], 10)
    ) {
      document.getElementById(selectedLocation).innerHTML = "";
      piecesArr[index].color == "black"
        ? outOfGamePiecesBlack.push(piecesArr[index])
        : outOfGamePiecesWhite.push(piecesArr[index]);
